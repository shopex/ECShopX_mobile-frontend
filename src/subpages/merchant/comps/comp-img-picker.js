    ...
    return (
        <View className={classNames('comps-img-picker', className)} onClick={onClick}>
            <View className="comps-img-picker-title">
                {required ? <Text className='required'>*</Text> : null}{title}
            </View>
            <View className={classNames('mt-16', 'comps-img-picker-content', 'border', { ismultiple })}>
                {
                    ismultiple
                        ? info.map((item, index) => {
                            ...
                            return <View className={classNames('view-flex view-flex-vertical view-flex-middle',{
                                'is-hasimg':uploadSuccess(index)
                            })}>
                                <SpImagePicker backgroundSrc={IMG_MAP[mode]} value={uploadSuccess(index)} size="small" onChange={handleChange(index)} uploadSuccess={uploadSuccess(index)}>
                                    <View className='picker-info'>{uploadSuccess(index)?'重新上传':item}</View>
                                </SpImagePicker> 
                            </View>
                        }) : <View className={classNames('view-flex view-flex-vertical view-flex-middle',{
                            'is-hasimg':uploadSuccess(0)
                        })}>
                            <SpImagePicker backgroundSrc={IMG_MAP[mode]} value={uploadSuccess(0)}  onChange={handleChange(0)} uploadSuccess={uploadSuccess(0)} >
