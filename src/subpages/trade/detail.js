...
import { ORDER_STATUS_TXT } from '@/consts'
      ...
      <View className='block-container trade-status'>
        {
          info && <View className='trade-status-desc'>
            <Text className={classNames("iconfont", ORDER_STATUS_TXT[info.orderStatus]?.icon)} />
            <Text className="status-desc">{ORDER_STATUS_TXT[info.orderStatus]?.label}</Text>
          </View>
        }
        {
