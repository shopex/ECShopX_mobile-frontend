...
function UgcNote(props) {
  ...
  const onEditImage = (item, index) => {
    Taro.navigateTo({
      url: `/subpages/mdugc/image-edit?image=${item}&index=${index}&topics=${JSON.stringify(
        moveArray[index] || []
      )}`
    })
  }
            ...
            {subjectList.map((item, index) => (
              <View className='subject-item' key={`subject-item__${index}`}>
                ...
                <Text
                  ...
                  onClick={handleDeleteSubject.bind(this, index)}
                ></Text>
              </View>
            ))}
          </View>
