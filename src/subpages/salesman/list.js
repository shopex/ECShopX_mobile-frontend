...
export default class TradeList extends Component {
  ...
  async fetch(params) {
    ...
    const {
      ...
      pager: { count: total },
      rate_status
    } = await api.trade.list({ ...params, isSalesmanPage: 1 })
  ...
  handleClickItem = (trade) => {
    ...
    let url = `/subpages/salesman/detail?id=${tid}`
  ...
  handleClickItemBtn = async (trade, type) => {
    ...
    let detailUrl = `/subpages/salesman/detail?id=${tid}`
    ...
    switch (type) {
      case 'cancel':
        Taro.navigateTo({
          url: `/subpages/salesman/cancel?order_id=${tid}`
        })
        break
      case 'rate':
        Taro.navigateTo({
