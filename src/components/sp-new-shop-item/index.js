...
const SpNewShopItem = (props) => {
  ...
  const isShowGoods =
    isShowGoodsProps && info.itemList && info.itemList.length > 0;
  ...
  const hasMore = useMemo(() => {
    ...
    return (
      (info.discountCardList || []).length > discountCount ||
      (info.marketingActivityList || []).length > activityCount
    );
  }, [
    info.discountCardList,
    info.marketingActivityList,
    discountCount,
    activityCount,
  ]);
  ...
  return inStore || inOrderList || inOrderDetail ? (
    ...
    <View className={classNames("sp-component-newshopitem", className)}>
          ...
          <View className={"sp-component-newshopitem-right-bottom"}>
            ...
            {marketingActivityList.map((item, index) => (
              ...
              >
                ...
                <View className={"right"}>
                  {hasMore && index === 0 && discountCardList.length === 0 && (
                    <View className={"right-arrow"} onClick={handleExpand}>
                      <Text
                        className={classNames("iconfont icon-arrowDown", {
                          ["expand"]: expand,
      ...
      {isShowGoods && (
        <View
          className={classNames("sp-component-newshopitem-good-list", {
            fill: info.itemList.length === goodCount,
          })}
        >
