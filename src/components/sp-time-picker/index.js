...
function SpTimePicker(props) {
  const { pickerIndex, activeTimeId, show= false, timeSlotsValueString = false, weekdays = [], timeSlots=[], onClose = voidFn,onChangeWeekDays= voidFn,onChangeTimeSlot = voidFn } = props
                ...
                <ScrollView className='week-container' scrollY>
                  {
                    ...
                    })} key={`weekday-item__${index}`} onClick={()=>onClickWeekDays(index)}>{item.title}</View>))
                  }
                </ScrollView>
                <ScrollView className='time-container' scrollY>
                  {
                    ...
                    })} key={`timeslot-item__${index}`} onClick={()=>onClickTimeSlot(item)}>{timeSlotsValueString ? item.value :`${item.value[0]} ~ ${item.value[1]}`}</View>))
                  }
                </ScrollView>
              </View>
            </SpFloatLayout>
