...
export default class GoodsBuyPanel extends Component {
  ...
  componentDidMount() {
    ...
    const skuDict = {};
    const originSpecIds =
      info.item_spec_desc && info.item_spec_desc.map((item) => item.spec_id);
    const newSpceItems =
      spec_items &&
      spec_items.sort((a, b) => {
  ...
  handleSelectSku = (item, idx, spec_name) => {
    ...
    const spec_full_text = `${spec_name}:${
      spec_value_name || spec_custom_value_name
    }`;
  ...
  handleBuyClick = async (type, skuInfo, num) => { 
    ...
    if (type === "fastbuy") {
      let pointitemUrlQuery = this.props.isPointitem
        ? `shop_id=0`
        : `shop_id=${id}`;
  ...
  initSelect = () => {
    ...
    for (let i = 0; i < last; i++) {
      if (
        this.skuDict &&
        this.skuDict[keyList[i]] &&
        this.skuDict[keyList[i]].store > 0
      ) {
                          ...
                          return (
                            <Text
                              className={classNames("sku-item", {
                                "is-active":
                                  sku.spec_value_id === selection[idx],
