...
export default class List extends Component {
  ...
  async fetch (params) {
    ...
    const {
      filterParams: { brand, category, start_price, end_price }
    } = this.state
    ...
    try {
      const [{ list, total_count }] = await Promise.all([api.pointitem.search(query)])
  ...
  render () {
    ...
    return (
      <View className='page-pointitem-home'>
        {isWeixin && (
          <CustomHeader isWhite={paddindTop > 0} isHome statusBarHeight={statusBarHeight} />
        )}
        ...
        <View
          className={classNames('filter-bar', { active: filterActive }, `goods-list__tabs filter`)}
          id='filter'
