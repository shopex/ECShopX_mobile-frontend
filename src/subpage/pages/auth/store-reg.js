            ...
            <View className='store-reg__input'>
              ...
              <View className='store-reg_time at-input'>
                ...
                <TimePicker onselctedTime={this.handleTime.bind(this, 0)}>
                  <Text className={classNames(!time[0] ? 'store-reg_time-placeholder' : '')}>
                    {time[0] ? time[0] : '开始时间'}
                  </Text>
                  <Text className={classNames(!time[0] ? 'store-reg_time-placeholder' : '')}>
                    -
                ...
                <TimePicker onselctedTime={this.handleTime.bind(this, 1)}>
                  <Text className={classNames(!time[1] ? 'store-reg_time-placeholder' : '')}>
                    {time[1] ? time[1] : '结束时间'}
                  </Text>
                </TimePicker>
              </View>
              ...
              <AtSwitch
                title='支持快递'
                checked={info.is_delivery}
                onChange={(e) => this.handleChange('is_delivery', e)}
              />
              <AtSwitch
                title='支持自提'
                checked={info.is_ziti}
                onChange={(e) => this.handleChange('is_ziti', e)}
              />
            </View>
