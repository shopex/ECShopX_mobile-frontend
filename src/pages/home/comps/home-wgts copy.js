...
export default class HomeWgts extends Component {
  ...
  fetch = ({ pageIndex, pageSize }) => {
    ...
    const twgt = wgts.slice(x - 3,  x > wgts.length ? wgts.length : x)
    log.debug(`${pageIndex}; ${pageSize}; ${wgts.length}; ${x - 3}; ${x > wgts.length ? wgts.length : x}`)
    this.setState({
      localWgts: [
        ...localWgts,
        ...twgt
  ...
  render () {
    ...
    return (
      <SpScrollView className='home-wgts' fetch={this.fetch} pageSize={3}>
