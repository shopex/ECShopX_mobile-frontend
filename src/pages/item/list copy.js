      ...
      <View className='page-goods-list'>
        ...
        {isNewGift ? (
          <View className='goods-list__toolbar1'>
            ...
            <View
              style={{
                display: `${!page.isLoading && !page.hasNext && !list.length ? 'none' : 'flex'}`,
                position: 'relative'
              }}
            >
              <FilterBar
        ...
        >
          ...
          {!page.isLoading && !page.hasNext && !list.length && (
            <SpNote
              img={
                isNewGift ? `${process.env.APP_IMAGE_CDN}/no_exist_product.png` : `trades_empty.png`
              }
              isUrl={isNewGift}
            >{`${isNewGift ? '此店铺不参加此次活动，看看别的吧' : '暂无数据～'}`}</SpNote>
          )}
