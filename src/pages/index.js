...
import { useLogin, useNavigation, useLocation, useModal, useWhiteShop, useStoreDivided } from '@/hooks'
import doc from '@/doc'
...
function Home() {
  ...
  const { checkStoreIsolation,getWhiteShop, connectWhiteShop, phoneCall } = useWhiteShop({
    onPhoneCallComplete: () => {
  ...
  const checkStoreIsolation2 = async () => {
    ...
    if (distributorId != shopInfo.distributor_id) {
      defalutShop = await api.shop.getShop()
