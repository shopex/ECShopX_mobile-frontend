...
const treeDay = 86400000 * 3; // 获取首页配置
...
const getHomeSetting = async () => {
  ...
  console.log('getHomeSetting1', echat); // 美洽客服配置

  Taro.setStorageSync('meiqia', meiqia); // 一洽客服配置

  Taro.setStorageSync('echat', echat); // 白名单配置、门店配置、图片存储信息、有数配置
...
class App extends Component {
  ...
  componentWillMount() {
    ...
    if (process.env.APP_PLATFORM === 'standard' && Taro.getEnv() === 'WEB') {// new LBS();
    } // 设置购物车默认类型
  ...
  config = {
    pages: ["pages/index", "pages/member/index"],
    subPackages: [],
    permission: {
      'scope.userLocation': {
  ...
  async init() {
    ...
    const {
      query = {}
    } = this.$router.params; // 初始化清楚s_smid
    ...
    if (query && query.scene || query.gu_user_id || query.smid) {
      const {
        ...
        chatId,
        gu_user_id = ''
      } = await normalizeQuerys(query); // 旧导购存放
      ...
      if (gu_user_id) {
        ...
        Taro.setStorageSync('gu_user_id', gu_user_id); // 如果是登录状态下打开分享且携带导购ID
    ...
    this.getSystemConfig(); // // 初始化tabbar
    // this.fetchTabs();
    // // 获取主题配色
    // this.fetchColors();
  }
  ...
  async getSystemConfig() {
    ...
    const [colorConfig, pointConfig] = await Promise.all([api.shop.getPageParamsConfig({
    ...
    const themeColor = colorConfig.list.length ? colorConfig.list[0].params : defaultColors; // 兼容老的主题
  ...
  componentDidCatchError() {} // 在 App 类中的 render() 函数没有实际作用
  // 请勿修改此函数
