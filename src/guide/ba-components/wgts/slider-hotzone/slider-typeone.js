  ...
  componentWillMount(){
    ...
    const {  info:{data} } = this.props
     Taro.getImageInfo({
       ...
       console.log('(Taro.$systemSize.screenWidth/ratio).toFixed(2)----1',(Taro.$systemSize.screenWidth/ratio).toFixed(2))
                  ...
                  >
                    ...
                    >
                         ...
                         <View className={`slider-wrap img-hotzone ${config.padded ? 'padded' : ''}`}>
                            {item.imgUrl&&<Image src={item.imgUrl} mode='widthFix' lazyLoad className={`img-hotzone_img ${config.rounded ? 'rounded' : null}`} />}
                              ...
                              item.hotzoneData.map((h_item, index) =>{
                                return (
                                  <View
                                    ...
                                    style={`width: ${h_item.widthPer*100}%; height: ${h_item.heightPer*100}%; top: ${h_item.topPer*100}%; left: ${h_item.leftPer*100}%`}
