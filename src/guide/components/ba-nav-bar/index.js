...
export default class BaNavBar extends Component {
    ...
    render() {
        const { navbarHeight, right, leftWidth, titleWidth, statusBarHeight, MenuButtonH }=this.state
        const {title, fixed, jumpType }=this.props
        if(!navbarHeight) return 

        return (
            <View className={classNames('ba-nav-bar',fixed?'ba-nav-bar__fixed':'')} style={styleNames({'height':`${navbarHeight}PX`,'padding-left':`${right}px`, 'padding-top': `${statusBarHeight}PX`})} >
                ...
                <View className='ba-nav-bar__title' style={styleNames({'width':`${titleWidth}PX`, 'height': `${MenuButtonH}PX`, 'line-height': `${MenuButtonH}PX`})}>
                    {title}
                </View>
            </View>
