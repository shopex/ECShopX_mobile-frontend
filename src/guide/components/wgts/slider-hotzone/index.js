                ...
                {config?
                    <View style={styleNames({ 'position': 'relative', 'top': `${Number(config.top) ? -(Number(config.top)) : 0}px` })}>
                        <View className={`wgt ${base.padded ? 'wgt__padded' : null} slidepostion`} style={styleNames({'background-image': 'url(' + config.bgimgUrl + ')', 'background-size': 'contain', 'padding': `${config.bgpadding?(`${config.bgpadding.toppadding||0}px ${config.bgpadding.rightpadding||0}px ${config.bgpadding.bottompadding||0}px ${config.bgpadding.leftpadding||0}px`):'0px'}` })}>
                            ...
                            {config.type == 'type4'&&<View className='slider-title'>
                                {data.length>0&&data.map((item,index)=>{
                                    return(
                                        <View key='index' className='slider-title__text' style={styleNames({'background-color':curIdx==index?config.contentBgColor:'#F9F9F8','color':curIdx==index?'#ffffff':'#999999'})} onClick={()=>this.changeCurrentIndex(index)}>
                                            {item.content||'未定义标题'}
                                        </View>
                                    )
                                })}
                            ...
                            {(config&&(config.type == 'type1'||config.type == 'type4' || !config.type)) && <SliderTypeOne info={info} autoPlay={autoPlay} curIdx={curIdx} onClick={this.handleClickItem} onChangeSwiper={this.handleChange}/>}
                            {(config&&config.type == 'type2')&&<SliderTypeTwo info={info} autoPlay={autoPlay} curIdx={curIdx} onClick={this.handleClickItem} onChangeSwiper={this.handleChange}   />}
                            {(config&&config.type == 'type3')&&<SliderTypeThree info={info} autoPlay={autoPlay} curIdx={curIdx} onClick={this.handleClickItem} onChangeSwiper={this.handleChange} />}
                            ...
                            {data.length > 1 && !config.dot &&config.shape!=='progressbar'&& (
                                <View className={classNames('slider-count',config.dotLocation, config.shape, config.dotColor)}>
                                    <View className='count'  style={styleNames({ 'background-color': config.dotColor })}>{curIdx + 1}/{data.length}</View>
                                </View>
                            )}
                            {data.length > 1&&config.dot &&config.shape==='progressbar'&&config.type == 'type2'&&(
                                <View className={classNames('slider-dot',config.dotLocation, config.shape)}>
                                  <View  className='slider-progressbar' style={`width:${progesswidth}rpx;background-color:${config.dotColor}`}>
                                      <View className='bar' style={styleNames({ 'background-color': config.activedotColor ,'left':`${progesswidth*percentage}rpx`})}></View>
                                  </View>
                              </View>
                            )}
