...
export default class WgtCoupon extends Component {
  ...
  componentDidMount(){
    ...
    let _this=this
    query.selectAll('.coupon-scroller__item-img').boundingClientRect( rec => {
      ...
      <View className={`wgt ${base.padded ? 'wgt__padded' : null}`}>
        ...
        <View>
          <View className='coupon-title'>
            <Text>先领券</Text>
            <Image className='coupon-title__img' mode='widthFix' src='/assets/imgs/like-coupon.png'></Image>
            <Text>更优惠</Text>
          </View>
          <View className={classNames('coupon-title__smtitle',data.length<4?'coupon-title__smtitle-smagrin':'')}>COUPONS</View>
          ...
          <View style={styleNames(config&&config.background ? { 'background': config.background } : null)}>
            {config&&config.style==='row'&&data&&data.length>3?<View className='coupon-scroller__padding'>
              ...
              > 
                <View 
                  className={classNames('coupon-scroller__item',data&&data.length>3?'coupon-scroller__item-padding1':'coupon-scroller__item-padding2')} 
                  
                > 
                  {data.map((item, idx) => {
                        return (
                        
                          <Image  key={item.item_id} className='coupon-scroller__item-img' mode='aspectFit'src={item.imgUrl} onClick={this.navigateTo.bind(this, `/guide/coupon-home/index?card_id=${item.id}`,item,idx)}></Image>
              ...
              {data&&data.length>3&& <View className='coupon-scroller__rightmask'>
                <View className='at-icon at-icon-chevron-right left-icon' onClick={this.handlenextscroll}></View>
              </View>}
              ...
              {data.map((item, idx) => {
                return (
                  <View className={classNames('coupon-wgt')} key='id'
                    onClick={this.navigateTo.bind(this, `/guide/coupon-home/index?card_id=${item.id}`,item,idx)}>
                    <Image className='coupon-wgt__image' mode='aspectFit'src={item.imgUrl}></Image>
