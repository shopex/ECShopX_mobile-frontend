...
import './crm-address.scss'

@connect(({ colors }) => ({
  colors: colors.current
}))

export default class CrmAddressIndex extends Component {
  constructor (props) {
    super(props)
  ...
  async fetch () {
    ...
    data.map(a_item => {
      if(a_item.third_data === this.$router.params.address_id) {
        this.setState({
          info: a_item
        })
      }
  ...
  handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(()=>{
        //Taro.navigateBack()
        Taro.navigateTo({
          url: `/marketing/pages/member/address`
        })
      }, 700)
    } catch (error) {
      return false
