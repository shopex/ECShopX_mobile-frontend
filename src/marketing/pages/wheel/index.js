...
export default class MarketingIndex extends Component {
  ...
  render() {
    let { info, rotateBg, rotateGift, loginNum, prizeData, dataForm, showPrize, showRegret, aniData } = this.state
    ...
    return (
      <View className='page-wheel-index'>
        <View className='background-img'>
          {
            dataForm.background_img ?
              <Image mode='widthFix' src={dataForm.background_img} /> : null
          }

        </View>
        <View className='wheel'>
          <View className='wheel-title'>
            {info.turntable_title}
          </View>
          ...
          <View className='wheel-turntable'>
            <View className='turntable' animation={aniData} style={{ 'box-shadow': `0px 0px 10rpx 10rpx ${dataForm.shadow_color}`, border: `10rpx solid ${dataForm.border_color}` }}>
              <View className='turntable-bg' style={{ transform: `rotate(${rotateBg}deg)` }}>
                {
                  list.map((item, index) => {
                    return (<View
                      key={`turntable${index}`}
                      style={{ transform: `rotate(${(360 / list.length) * (index + 1)}deg)`, background: `${dataForm.line_color}` }}
                    ></View>)
                  })
                }
            ...
            <View className='wheel-turntable__pointer'>
              {
                dataForm.pointer_img ?
                  <Image onClick={this.handleClickLotteryDraw.bind(this)} mode='widthFix' src={dataForm.pointer_img} />
                  : null
              }
            </View>
          </View>
        ...
        <AtModal isOpened={showPrize} onClose={() => { this.setState({ showPrize: true }) }} className='wheel-prize' >
        ...
        <AtModal isOpened={showRegret} onClose={() => { this.setState({ showRegret: true }) }} className='wheel-prize' >
          <View className='wheel-prize__modal'>
            <Image
              style='width:100%;'
            ...
            <View className='wheel-prize__modal-con'>
              ...
              <AtButton className='button-regret' onClick={() => { this.setState({ showRegret: false }) }}>继续努力</AtButton>
            </View>
          </View>
        </AtModal>
      </View>
