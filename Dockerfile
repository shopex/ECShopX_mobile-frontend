...
COPY package*.json ./
COPY .env ./
RUN npm config set registry https://registry.npmmirror.com && npm config set sass_binary_site=https://npm.taobao.org/mirrors/node-sass/ && npm config set @shopex:registry http://registry.npm.ishopex.cn && npm ci
