...
var ISO8061Date = require("../util/iso8061date.js");
...
exports.serialize = function (signedIdentifiersJs) {
  ...
  if (keys.length > 0) {
    keys.forEach(function (key) {
      ...
      doc = doc.ele(AclConstants.SIGNED_IDENTIFIER_ELEMENT).ele(AclConstants.ID).txt(key).up().ele(AclConstants.ACCESS_POLICY);
      ...
      if (accessPolicy.Start) {
        ...
        doc = doc.ele(AclConstants.START).txt(startIsoString).up();
      }
      ...
      if (accessPolicy.Expiry) {
        ...
        doc = doc.ele(AclConstants.EXPIRY).txt(expiryIsoString).up();
      }

      if (accessPolicy.Permissions) {
        doc = doc.ele(AclConstants.PERMISSION).txt(accessPolicy.Permissions).up();
      }
